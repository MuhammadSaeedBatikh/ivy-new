"""Collection of Mindspore gradient functions, wrapped to fit Ivy syntax and signature."""
...
def variable(x, /):
    return ms.Parameter(x, requires_grad=True)
...
def variable_data(x: ms.Parameter, /) -> ms.Parameter:
    return x.value()
...
def execute_with_gradients(
    ...
    *,
    ...
    if isinstance(y, ivy.NativeArray):
        ...
        _grads = grad_fn(xs_required)
        grads = _set_duplicates(_grads, required_duplicate_index_chains)
    else:
...
def value_and_grad(func):
    ...
    def callback_fn(xs):
        ...
        value, grad = ops.value_and_grad(grad_fn)(xs)
...
def grad(func: Callable):
    ...
    def callback_fn(x_in):
        ms_grad = ops.grad(grad_fn)
        x = ivy.to_native(x_in)
        x.requires_grad = True
        grad_val = ms_grad(x)
        return ivy.to_ivy(grad_val)
