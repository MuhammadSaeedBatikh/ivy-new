...
class Finfo:
    def __init__(self, mindspore_finfo: np.finfo):
        self._mindspore_finfo = mindspore_finfo

    def __repr__(self):
        return repr(self._mindspore_finfo)

    @property
    def bits(self):
        return self._mindspore_finfo.bits

    @property
    def eps(self):
        return float(self._mindspore_finfo.eps)

    @property
    def max(self):
        return float(self._mindspore_finfo.max)

    @property
    def min(self):
        return float(self._mindspore_finfo.min)

    @property
    def smallest_normal(self):
        return float(self._mindspore_finfo.tiny)
...
class Iinfo:
    def __init__(self, mindspore_iinfo: None):
        self.mindspore_iinfo = mindspore_iinfo
...
def astype(
    ...
    if x.dtype == dtype:
        return msnp.copy(x) if copy else x
    return F.cast(x, dtype)


def broadcast_arrays(*arrays: ms.Tensor) -> List[ms.Tensor]:
    return msnp.broadcast_arrays(*arrays)


@with_unsupported_dtypes({"2.0.0 and below": ("bfloat16",)}, backend_version)
def broadcast_to(
        x: ms.Tensor,
        /,
    ...
    if x.ndim > len(shape):
        return msnp.broadcast_to(x.reshape(-1), shape)
    return msnp.broadcast_to(x, shape)


@_handle_nestable_dtype_info
@with_unsupported_dtypes({"1.11.0 and below": ("bfloat16",)}, backend_version)
def finfo(type: Union[ms.Type, str, ms.Tensor], /) -> Finfo:
    if isinstance(type, ms.Tensor):
        type = str(type.dtype).lower()
    elif isinstance(type, ms.Type):
        type = str(type).lower()
    if "bfloat16" in type:
...
def iinfo(type: Union[ms.Type, str, ms.Tensor], /) -> Iinfo:
    if isinstance(type, ms.Tensor):
        type = str(type.dtype).lower()
    elif isinstance(type, ms.Type):
        type = str(type).lower()
...
def result_type(
    ...
    if len(arrays_and_dtypes) <= 1:
        return msnp.result_type(arrays_and_dtypes)

    result = msnp.result_type(
        arrays_and_dtypes[0], arrays_and_dtypes[1]
    )
    for i in range(2, len(arrays_and_dtypes)):
        result = msnp.result_type(result, arrays_and_dtypes[i])
